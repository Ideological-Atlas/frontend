...
export function Button({
  ...
  const createRipple = (event: MouseEvent<HTMLButtonElement>) => {
    ...
    // Calculamos la posición para que el centro del ripple sea el cursor
    ...
    // Llamamos al onClick original si existe
    if (onClick) {
      onClick(event);
    }
  };
  ...
  const variants = {
    ...
    outline:
      "bg-transparent border border-slate-300 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-900 dark:text-white h-12 px-6 text-base",
  };
  ...
  return (
    <button
      ...
      {...props}
    >
      <span className="relative z-10 pointer-events-none truncate">{children}</span>
